...
function InventoryOpsSection({ damageLossOverview = { rows: [], total: 0, summary: {} }, formatBs }) {
  ...
  const generateReport = () => {
    ...
    const recoveredRentalIds = uniqueRentalIds.filter((rentalId) => (
      moneyNumber(economicsByRental?.[rentalId]?.totalRecoveredBs) > 0.009
    ));

    const financialRows = recoveredRentalIds.map((rentalId, index) => {
      ...
      const entries = Array.isArray(economic?.recoveryBreakdown) ? economic.recoveryBreakdown : [];
      const contractIssues = filteredRows.filter((row) => String(row?.rentalId ?? '') === rentalId);
      const issueSummary = contractIssues.map((row) => (
        `${row.lossType === 'danado' ? 'Daño' : 'Faltante'}: ${row.quantity} × ${row.itemName}`
      )).join(' · ');
      const recoveryLines = entries.map((entry) => {
        const method = String(entry?.paymentMethod ?? '').trim().toLowerCase();
        const account = String(entry?.paymentAccount ?? '').trim();
        const destination = entry?.source === 'guarantee'
          ? 'Garantía del contrato'
          : method === 'qr' || method === 'transferencia'
            ? (account || 'Cuenta no especificada')
            : 'Caja / efectivo';
        const methodLabel = entry?.source === 'guarantee'
          ? 'GARANTÍA'
          : method === 'qr'
            ? 'QR'
            : method === 'transferencia'
              ? 'TRANSFERENCIA'
              : 'EFECTIVO';
        const receipt = entry?.receiptCode ? `<div><span>RECIBO / REF.</span><strong>${escapeHtml(entry.receiptCode)}</strong></div>` : '';
        const when = entry?.occurredAt ? `<div><span>FECHA</span><strong>${escapeHtml(formatDateTime(entry.occurredAt))}</strong></div>` : '';
        const by = entry?.registeredBy ? `<div><span>REGISTRADO POR</span><strong>${escapeHtml(entry.registeredBy)}</strong></div>` : '';
        return `<div class="payment-detail">
          <div class="payment-method"><span>FORMA DE COBRO</span><strong>${escapeHtml(methodLabel)}</strong></div>
          <div><span>MONTO</span><strong>${escapeHtml(formatBs(entry?.amountBs))}</strong></div>
          <div><span>DESTINO / CUENTA</span><strong>${escapeHtml(destination)}</strong></div>
          ${when}${by}${receipt}
        </div>`;
      }).join('');
      return `
        <div class="finance-record ${['paid', 'guarantee', 'mixed'].includes(status.className) ? 'is-paid' : 'is-partial'}">
          <div class="finance-no">${index + 1}</div>
          <div class="finance-main">
            <div class="finance-headline">
              <div><span>CONTRATO / ORDEN</span><strong>${escapeHtml(firstRow?.contractCode || '-')}</strong><small>${escapeHtml(firstRow?.orderCode || '')}</small></div>
              <div><span>CLIENTE</span><strong>${escapeHtml(firstRow?.customerName || '-')}</strong></div>
              <div><span>CARGO</span><strong>${escapeHtml(formatBs(economic?.clientChargedBs))}</strong></div>
              <div class="recovered"><span>RECUPERADO</span><strong>${escapeHtml(formatBs(economic?.totalRecoveredBs))}</strong></div>
              <div class="pending"><span>PENDIENTE</span><strong>${escapeHtml(formatBs(economic?.pendingRecoveryBs))}</strong></div>
              <div class="finance-status"><span class="collection ${status.className}">${escapeHtml(status.label)}</span></div>
            </div>
            <div class="issue-summary"><span>INCIDENCIAS</span><strong>${escapeHtml(issueSummary || 'Sin detalle de incidencias')}</strong></div>
            <div class="payment-details">${recoveryLines}</div>
          </div>
        </div>`;
    }).join('');
...
<div class="section-title">2. Cobros y recuperaciones realizadas</div>
<div class="finance-list">${financialRows || '<div class="note">No hay cobros ni recuperaciones registrados para los filtros seleccionados.</div>'}</div>
...
<div class="footer"><span>EL COPETÍN · Inventario</span><span>${filteredRows.length} incidencia(s) · ${recoveredRentalIds.length} contrato(s) con recuperación</span></div>
</main></body></html>`;
    setFeedback('');
    setReportPreview({
