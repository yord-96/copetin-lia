...
function InventoryDashboardSection({
  ...
  const [movementHistoryRows, setMovementHistoryRows] = useState(null);
  const [movementHistoryTotal, setMovementHistoryTotal] = useState(0);
  ...
  const [legacyForm, setLegacyForm] = useState({ contractCode: '', contractDate: '', clientId: '', customerName: '', responsibleName: '', eventName: '', commercialPendingBs: '', guaranteePendingBs: '', guaranteeHeldBs: '', refundDueBs: '', notes: '', items: [] });
  ...
  const openLegacyCreate = async () => {
    ...
    setLegacyForm({ contractCode: '', contractDate: '', clientId: '', customerName: '', responsibleName: currentUser?.name ?? '', eventName: '', commercialPendingBs: '', guaranteePendingBs: '', guaranteeHeldBs: '', refundDueBs: '', notes: '', items: [] });
  ...
  const openLegacyEdit = async (row) => {
    ...
    setLegacyForm({
      ...
      guaranteePendingBs: String(row.guaranteePendingBs ?? ''),
  ...
  const printLegacyContractDocument = (legacy) => {
    if (!legacy) return;
    const popup = window.open('', '_blank', 'width=980,height=760');
    if (!popup) { setFeedback('Chrome bloqueó la ventana del documento. Habilita las ventanas emergentes.'); setFeedbackType('error'); return; }
    const itemRows = (Array.isArray(legacy.items) ? legacy.items : []).map((line) => `<tr><td>${escapeHtml(line.itemName)}</td><td>${Number(line.quantity || 0)}</td><td>${escapeHtml(line.status === 'pending_return' ? 'Pendiente devolución' : line.status === 'missing' ? 'Faltante' : line.status === 'damaged' ? 'Dañado' : line.status === 'returned' ? 'Devuelto' : 'Resuelto')}</td><td>${escapeHtml(formatBs(line.chargeBs || 0))}</td><td>${escapeHtml(line.note || '-')}</td></tr>`).join('');
    popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Contrato rezagado ${escapeHtml(legacy.contractCode)}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#172033;margin:0}header{border-bottom:3px solid #173a70;padding-bottom:14px}h1{margin:5px 0;color:#173a70}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin:16px 0}.meta div,.money div{border:1px solid #dbe2ea;border-radius:8px;padding:10px}.meta small,.money small{display:block;color:#64748b}.money{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin:14px 0}.money strong{color:#173a70}table{width:100%;border-collapse:collapse;margin-top:16px}th{background:#173a70;color:#fff;text-align:left;padding:8px;font-size:11px}td{border:1px solid #dbe2ea;padding:8px;font-size:11px}.notes{margin-top:14px;padding:12px;background:#f8fafc}.actions{text-align:right;margin-top:16px}.actions button{border:0;border-radius:8px;background:#e84a00;color:#fff;padding:10px 16px;font-weight:700}@media print{.actions{display:none}}</style></head><body><header><small>EL COPETÍN · DOCUMENTO INDIVIDUAL</small><h1>Contrato rezagado ${escapeHtml(legacy.contractCode)}</h1><div>${escapeHtml(legacy.customerName)} · ${escapeHtml(legacy.contractDate)}</div></header><section class="meta"><div><small>Responsable</small><strong>${escapeHtml(legacy.responsibleName || 'Sin responsable')}</strong></div><div><small>Evento / referencia</small><strong>${escapeHtml(legacy.eventName || '-')}</strong></div></section><section class="money"><div><small>Saldo comercial</small><strong>${escapeHtml(formatBs(legacy.commercialDueBs || 0))}</strong></div><div><small>Daños / pérdidas</small><strong>${escapeHtml(formatBs(legacy.damageDueBs || 0))}</strong></div><div><small>Garantía por cobrar</small><strong>${escapeHtml(formatBs(legacy.guaranteePendingBs || 0))}</strong></div><div><small>Garantía retenida</small><strong>${escapeHtml(formatBs(legacy.guaranteeHeldBs || 0))}</strong></div><div><small>Por devolver</small><strong>${escapeHtml(formatBs(legacy.refundableGuaranteeBs || 0))}</strong></div></section><table><thead><tr><th>Ítem</th><th>Cantidad</th><th>Situación</th><th>Cargo</th><th>Nota</th></tr></thead><tbody>${itemRows || '<tr><td colspan="5">Sin material pendiente registrado.</td></tr>'}</tbody></table><div class="notes"><strong>Notas</strong><p>${escapeHtml(legacy.notes || 'Sin notas.')}</p></div><div class="actions"><button onclick="window.print()">Imprimir / guardar PDF</button></div></body></html>`);
  ...
  useEffect(() => {
    if (!isMovementsModule) {
      setMovementHistoryRows(null);
      setMovementHistoryTotal(0);
      return undefined;
    }
    let cancelled = false;
    const timerId = window.setTimeout(() => {
      setMovementHistoryLoading(true);
      api.inventory.searchMovementHistory({
        query: deferredQuery,
        from: dateFrom,
        to: dateTo,
        type: movementTypeFilter,
        user: movementUserFilter,
        limit: INVENTORY_MOVEMENT_RENDER_LIMIT,
        offset: 0,
      })
        .then((payload) => {
          if (cancelled) return;
          setMovementHistoryRows(Array.isArray(payload?.rows) ? payload.rows : []);
          setMovementHistoryTotal(Number(payload?.total ?? 0));
        })
        .catch((error) => {
          if (cancelled) return;
          setFeedback(error?.message || 'No se pudo buscar el historial completo de inventario.');
          setFeedbackType('error');
        })
        .finally(() => {
          if (!cancelled) setMovementHistoryLoading(false);
        });
    }, deferredQuery ? 220 : 0);
    return () => {
      cancelled = true;
  ...
  const inventoryOrdersPageCount = Math.max(1, Math.ceil(filteredPrepOrderRows.length / inventoryOrdersPageSize));
  const safeInventoryOrdersPage = Math.min(inventoryOrdersPage, inventoryOrdersPageCount);
  const visiblePrepOrderRows = inventoryFiltersAreCleared
    ? showAllInventoryOrders
      ? filteredPrepOrderRows.slice(
          (safeInventoryOrdersPage - 1) * inventoryOrdersPageSize,
          safeInventoryOrdersPage * inventoryOrdersPageSize,
        )
  ...
  const openOperationalRangeReport = () => {
    ...
    const getReturnIssueCounts = (row) => {
      ...
      return { damaged, missing: missingFromReview, pendingClient };
    };
    const classify = (row) => {
      const status = normalizeText(row?.inventoryStatus);
      const returnStatus = normalizeText(row?.returnReview?.status);
      const { damaged, missing, pendingClient } = getReturnIssueCounts(row);
      const hasIssues = damaged > 0 || missing > 0 || returnStatus === 'issues' || Boolean(row?.revisionAlert);
      const withClient = pendingClient > 0 || returnStatus === 'left_with_client' || Boolean(row?.clientPendingPickup);
      if (status === 'devuelto') {
...
<body>
  ...
  const movementRows = useMemo(() => {
    ...
    const movementSource = Array.isArray(movementHistoryRows) ? movementHistoryRows : inventoryMovements;
    const sortedInventoryMovements = movementSource
      .slice()
    ...
    const persistedRows = visibleInventoryMovements.map((movement) => {
      ...
      return {
        ...
        sku: movement.sku ?? itemRow?.sku ?? (String(movement.itemId ?? '').replace(/[^a-zA-Z0-9]/g, '').slice(0, 7).toUpperCase() || 'COD'),
    ...
    const hasExplicitMovementFilter = Boolean(
      deferredQuery.trim()
      || dateFrom
      || dateTo
    ...
    const serviceOrderRows = hasExplicitMovementFilter ? [] : rentalsForTrace.flatMap((rental) => {
  ...
  }, [activeRentals, cancelledRentals, contracts, inventoryMovements, inventoryRows, movementHistoryRows, deferredQuery, dateFrom, dateTo, movementTypeFilter, movementUserFilter]);
    ...
    setReceivingModal((current) => {
      ...
      return {
        ...
        items: current.items.map((line) => {
          ...
          if (['damagedQty', 'missingQty', 'pendingClientQty'].includes(field)) {
            const damagedQty = Math.max(0, Math.min(expectedQty, Math.trunc(Number(nextLine.damagedQty ?? 0))));
            const missingQty = Math.max(0, Math.min(expectedQty, Math.trunc(Number(nextLine.missingQty ?? 0))));
            const pendingClientQty = Math.max(0, Math.min(expectedQty, Math.trunc(Number(nextLine.pendingClientQty ?? 0))));
            nextLine.damagedQty = damagedQty;
            nextLine.missingQty = missingQty;
            nextLine.pendingClientQty = pendingClientQty;
            nextLine.returnedQty = Math.max(0, expectedQty - damagedQty - missingQty - pendingClientQty);
          } else if (field === 'returnedQty') {
            const returnedQty = Math.max(0, Math.min(expectedQty, Math.trunc(Number(nextLine.returnedQty ?? 0))));
            const damagedQty = Math.max(0, Math.trunc(Number(nextLine.damagedQty ?? 0)));
            const pendingClientQty = Math.max(0, Math.trunc(Number(nextLine.pendingClientQty ?? 0)));
            nextLine.returnedQty = returnedQty;
            nextLine.missingQty = Math.max(0, expectedQty - returnedQty - damagedQty - pendingClientQty);
          }
  ...
  const getReceivingLineNumbers = (line) => {
    ...
    const pendingClientQty = Math.max(0, Math.trunc(Number(line.pendingClientQty ?? 0)));
    ...
    const balanceQty = expectedQty - returnedQty - damagedQty - missingQty - pendingClientQty;
    return {
      expectedQty,
      returnedQty,
      damagedQty,
  ...
  const receivingTotals = useMemo(() => {
    ...
    return receivingModal.items.reduce((totals, line) => {
      ...
      if (values.damagedQty > 0 || values.missingQty > 0 || values.pendingClientQty > 0) totals.issueRows += 1;
      totals.pendingClientUnits += values.pendingClientQty;
      totals.damagedUnits += values.damagedQty;
      totals.missingUnits += values.missingQty;
  ...
  const submitReceiving = async (event) => {
    ...
    const normalizedItems = receivingModal.items.map((line) => {
      const values = getReceivingLineNumbers(line);
      return { line, values };
    });
    const invalidLine = normalizedItems.find(({ values }) => values.balanceQty !== 0);
    if (invalidLine) {
      setReceivingError(`Revisa "${invalidLine.line.itemName}": bueno + dañado + faltante + con cliente debe sumar ${invalidLine.values.expectedQty}.`);
      return;
    }
    const lineWithoutNote = normalizedItems.find(({ line, values }) => (
      (values.damagedQty > 0 || values.missingQty > 0 || values.pendingClientQty > 0)
      && !String(line.damageNote ?? '').trim()
    ));
    if (lineWithoutNote) {
      setReceivingError(`Describe la novedad de "${lineWithoutNote.line.itemName}".`);
      return;
    }

    const pendingItems = normalizedItems
      .filter(({ values }) => values.pendingClientQty > 0)
      .map(({ line, values }) => ({
        lineKey: line.lineKey,
        itemId: line.itemId,
        itemName: line.itemName,
        expectedQty: values.expectedQty,
        pendingQty: values.pendingClientQty,
        note: String(line.damageNote ?? '').trim(),
      }));
    const hasPendingClient = pendingItems.length > 0;
    const hasDamageOrMissing = normalizedItems.some(({ values }) => values.damagedQty > 0 || values.missingQty > 0);
    const returnStatus = hasPendingClient ? 'left_with_client' : hasDamageOrMissing ? 'issues' : 'complete';
    const generalNote = String(receivingModal.notes ?? '').trim();
    const pendingNote = generalNote || pendingItems.map((line) => `${line.itemName}: ${line.pendingQty} pendiente(s)${line.note ? ` · ${line.note}` : ''}`).join(' | ');

    setIsReceiving(true);
    const currentRentalId = receivingModal.rental.id;
    setReceivingModal(null);
    setReturnProcessingMessage(hasPendingClient ? 'Registrando retorno parcial...' : 'Cerrando recepción de inventario...');

    setOperationalOverrides((current) => ({
      ...current,
      [currentRentalId]: {
        ...(current[currentRentalId] ?? {}),
        inventoryStatus: hasPendingClient ? 'salio' : 'devuelto',
        returnReview: { status: returnStatus, note: generalNote },
        clientPendingPickup: hasPendingClient ? { active: true, note: pendingNote, items: pendingItems } : null,
        ...(hasPendingClient ? {} : { inventoryReturnedAt: new Date().toISOString() }),
      },
    }));
    ...
    try {
      ...
      await onReceiveReturnedOrder?.({
        ...
        partialReturn: hasPendingClient,
        returnReview: { status: returnStatus, note: generalNote },
        clientPendingPickup: hasPendingClient
          ? { active: true, note: pendingNote, items: pendingItems }
          : { active: false, note: '' },
        returnedItems: normalizedItems.map(({ line, values }) => ({
          ...line,
          returnedQty: values.returnedQty,
          damagedQty: values.damagedQty,
          missingQty: values.missingQty,
          pendingClientQty: values.pendingClientQty,
      ...
      showMessage(
        hasPendingClient
          ? `Retorno parcial registrado: ${receivingTotals.pendingClientUnits} unidad(es) siguen con el cliente.`
          : hasDamageOrMissing
            ? 'Recepción registrada con daños/faltantes.'
            : 'Recepción de inventario registrada correctamente.',
      );
    } catch (error) {
      setOperationalOverrides((current) => {
        const next = { ...current };
      ...
      setReceivingError(error?.message || 'No se pudo registrar la recepción.');
    } finally {
  ...
  return (
    <section className={`panel inventory-dashboard ${moduleViewClass}`}>
          ...
          {isMovementsModule ? (
            ...
            <article className="inventory-ops-card">
              ...
              <div className="inventory-ops-list">
                ...
                {visiblePrepOrderRows.map((row) => (
                  <div key={row.id} className="inventory-ops-row">
                    ...
                    <div className="inventory-ops-state">
                      ...
                      <strong>{row.clientPendingPickup ? 'Retorno parcial' : row.inventoryStatus === 'salio' ? 'Fuera de almacen' : row.inventoryStatus === 'confirmado' ? 'Lista para salir' : row.inventoryStatus === 'devuelto' ? 'Devuelto' : 'Pendiente'}</strong>
                    ...
                    <div className="inventory-ops-actions">
                      ...
                      <div className="inventory-ops-process" aria-label={`Proceso de inventario para ${row.contractCode}`}>
                        ...
                        <button
                          type="button"
                          className={`inventory-ops-check ${row.inventoryStatus === 'devuelto' ? 'done' : row.clientPendingPickup ? 'pending-client' : ''}`}
                          style={row.clientPendingPickup ? { borderColor: '#f59e0b', background: '#fff7ed', color: '#b45309' } : undefined}
                          disabled={!row.canConfirmInventory || row.inventoryStatus !== 'salio'}
                          onClick={() => handleInventoryOrderAction({ ...row, inventoryAction: 'return' })}
                        >
                          <span aria-hidden="true">{row.inventoryStatus === 'devuelto' ? '✓' : row.clientPendingPickup ? '◐' : ''}</span>
                          {row.clientPendingPickup ? 'Volvió parcial' : 'Volvió'}
                        </button>
                      </div>
                      <div
                ...
                {inventoryFiltersAreCleared && filteredPrepOrderRows.length > 7 ? (
                  <div className="inventory-ops-view-all">
                    {showAllInventoryOrders ? (
                      <>
                        <span>
                          Mostrando {(safeInventoryOrdersPage - 1) * inventoryOrdersPageSize + 1}–
                          {Math.min(safeInventoryOrdersPage * inventoryOrdersPageSize, filteredPrepOrderRows.length)} de {filteredPrepOrderRows.length} órdenes.
                        </span>
                        <div className="inventory-ops-pagination" aria-label="Paginación de órdenes operativas">
                          <button
                            type="button"
                            className="ghost-button"
                            disabled={safeInventoryOrdersPage <= 1}
                            onClick={() => setInventoryOrdersPage(Math.max(1, safeInventoryOrdersPage - 1))}
                          >
                            Anterior
                          </button>
                          <span>Página {safeInventoryOrdersPage} de {inventoryOrdersPageCount}</span>
                          <button
                            type="button"
                            className="ghost-button"
                            disabled={safeInventoryOrdersPage >= inventoryOrdersPageCount}
                            onClick={() => setInventoryOrdersPage(Math.min(inventoryOrdersPageCount, safeInventoryOrdersPage + 1))}
                          >
                            Siguiente
                          </button>
                        </div>
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="ghost-button"
                          onClick={() => {
                            setInventoryOrdersPage(1);
                            setShowAllInventoryOrders(true);
                          }}
                        >
                          Mostrar 25 por página
                        </button>
                      </>
                    )}
                  </div>
            ...
            <article className="inventory-ops-card inventory-legacy-card">
              ...
              <div className="inventory-legacy-table-wrap"><table className="inventory-legacy-table"><thead><tr><th>Contrato</th><th>Fecha</th><th>Cliente</th><th>Responsable</th><th>Material pendiente</th><th>Economia</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
                {filteredLegacyContracts.map((row, rowIndex) => {
                  ...
                  return <tr key={row.id}><td><strong>{row.contractCode}</strong><small>REZAGADO</small></td><td>{row.contractDate}</td><td><strong>{row.customerName}</strong></td><td>{row.responsibleName}</td><td><strong>{row.pendingUnits} u.</strong><small>{row.pendingItemCount} incidencia(s)</small></td><td><strong>{row.totalDueBs > 0 ? `Por cobrar ${formatBs(row.totalDueBs)}` : row.refundableGuaranteeBs > 0 ? `Por devolver ${formatBs(row.refundableGuaranteeBs)}` : row.guaranteePendingBs > 0 ? `Garantía por cobrar ${formatBs(row.guaranteePendingBs)}` : 'Sin saldo'}</strong><small>Saldo {formatBs(row.commercialDueBs || 0)} · Daños {formatBs(row.damageDueBs || 0)} · Garantía retenida {formatBs(row.guaranteeHeldBs || 0)}</small></td><td><span className={`inventory-legacy-status ${row.isResolved ? 'resolved' : 'pending'}`}>{row.isResolved ? 'Finalizado' : 'Pendiente'}</span></td><td><div className="inventory-legacy-row-actions"><button type="button" className="ghost-button" onClick={() => setLegacyDetail(row)}>Abrir</button><div className="inventory-actions-menu-wrap" ref={rowMenuOpenId === menuId ? rowMenuRef : null}><button type="button" className="inventory-row-menu-button" aria-label={`Más opciones para el contrato ${row.contractCode}`} aria-expanded={rowMenuOpenId === menuId} onClick={(event) => toggleRowMenu(menuId, event, openUp)}>⋮</button>{rowMenuOpenId === menuId ? renderLegacyRowMenu(row, openUp) : null}</div></div></td></tr>;
                })}
            ...
            <footer className="inventory-table-footer-modern">
              <span>
                {isProductsModule && productKardexLoading
                  ...
                  : isMovementsModule && movementHistoryLoading
                    ? 'Buscando en todo el historial del VPS...'
                    : isMovementsModule
                      ? `Mostrando ${pagedRows.length} de ${filteredRows.length} cargados · ${movementHistoryTotal || filteredRows.length} coincidencia(s) en todo el historial`
                      : `Mostrando ${pagedRows.length} de ${filteredRows.length} ${isAdjustModule ? 'ajustes' : isCombosModule ? 'combos' : isCategoriesModule ? 'categorias' : 'productos'}`}
              </span>
      ...
      {operationalReportRow ? (() => {
        ...
        const returnStatusLabel = pendingClientTotal > 0
          ...
          : operationalReportRow.inventoryStatus === 'devuelto'
            ...
            : 'Retorno pendiente';
        const finalTone = pendingClientTotal > 0
          ? 'warning'
          : operationalReportRow.inventoryStatus === 'devuelto'
            ? (damagedTotal > 0 || missingTotal > 0 ? 'warning' : 'success')
            : 'pending';
        ...
        const timeline = [
          ...
          },
          {
            ...
            done: Boolean(operationalReportRow.inventoryReturnedAt) || reportInventoryStatus === 'devuelto' || pendingClientTotal > 0,
          },
        ];
        const metrics = [
          ...
          { label: 'Faltante', value: missingTotal, tone: missingTotal > 0 ? 'danger' : 'muted' },
          { label: 'Con cliente', value: pendingClientTotal, tone: pendingClientTotal > 0 ? 'warning' : 'muted' },
        ];
        return (
              ...
              <div className="inventory-report-body">
                ...
                <section className="inventory-report-panel">
                  <div className="inventory-report-section-head">
                    ...
                    <div className={`inventory-report-inline-status ${missingTotal > 0 || damagedTotal > 0 || pendingClientTotal > 0 ? 'warning' : 'success'}`}>
                      {returnedGoodTotal} bien · {damagedTotal} dañados · {missingTotal} faltantes · {pendingClientTotal} con cliente
                    </div>
                  </div>
                  <div className="inventory-report-table-wrap">
                    <table className="inventory-report-table">
                ...
                {pendingClientItems.length > 0 ? (
                  <section className="inventory-report-panel" style={{ borderColor: '#f5c26b', background: '#fffaf0' }}>
                    <div className="inventory-report-section-head">
                      <div>
                        <span>Pendiente con cliente</span>
                        <h4>Material que todavía no volvió</h4>
                      </div>
                      <div className="inventory-report-inline-status warning">{pendingClientTotal} unidad(es) pendientes</div>
                    </div>
                    <div className="inventory-report-table-wrap">
                      <table className="inventory-report-table">
                        <thead><tr><th>Ítem</th><th>Pendiente</th><th>Observación</th></tr></thead>
                        <tbody>
                          {pendingClientItems.map((line, index) => (
                            <tr key={line.lineKey || `${line.itemId}-${index}`}>
                              <td><strong>{line.itemName || 'Ítem'}</strong></td>
                              <td><span className="inventory-report-number warning">{Math.max(0, Number(line.pendingQty ?? 0))}</span></td>
                              <td>{line.note || operationalReportRow.clientPendingPickup?.note || 'Pendiente de recojo al cliente'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            <div className="orders-preview-body inventory-receiving-body">
              <div className="inventory-ops-return-mode">
                <div className="form-field">
                  ...
                  <div style={{ minHeight: 44, border: '1px solid #d7dee8', borderRadius: 9, background: '#fff', padding: '8px 11px', display: 'grid', gap: 2 }}>
                    <strong style={{ color: receivingTotals.pendingClientUnits > 0 ? '#b45309' : receivingTotals.damagedUnits > 0 || receivingTotals.missingUnits > 0 ? '#b42318' : '#15803d' }}>
                      {receivingTotals.pendingClientUnits > 0
                        ? `Retorno parcial · ${receivingTotals.pendingClientUnits} con cliente`
                        : receivingTotals.damagedUnits > 0 || receivingTotals.missingUnits > 0
                          ? 'Retorno con novedades'
                          : 'Retorno completo'}
                    </strong>
                    <small style={{ color: '#667085' }}>Se calcula automáticamente con el detalle de cada ítem.</small>
                  </div>
                </div>
                <label className="form-field">
                  <span>Nota general <small style={{ fontWeight: 500, color: '#98a2b3' }}>(opcional)</small></span>
                  <textarea
                    rows={3}
                    value={receivingModal.notes}
                    onChange={(event) => setReceivingModal((current) => (current ? { ...current, notes: event.target.value } : current))}
                    placeholder="Observación general del retorno. El detalle específico se registra en cada ítem."
                  />
                </label>
              </div>
              ...
              <div className="inventory-receiving-summary" style={{ gridTemplateColumns: 'repeat(5, minmax(0, 1fr))' }}>
                ...
                <span><small>Pérdida interna</small><strong>{formatBs(receivingTotals.internalPenaltyBs)}</strong></span>
              </div>
              <div className="transport-checklist-table">
                <div className="transport-checklist-head inventory-receiving-head" style={{ gridTemplateColumns: 'minmax(190px,1.45fr) 58px 58px 58px 58px 72px 112px minmax(112px,.8fr) 82px minmax(150px,1fr)' }}>
                  <span>Ítem</span>
                  <span>Esperado</span>
                  <span>Bueno</span>
                  <span>Dañado</span>
                  <span>Faltante</span>
                  <span>Con cliente</span>
                  <span>Precio</span>
                  <span>Origen</span>
                  <span>Total</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <input
                        ...
                        value={line.pendingClientQty ?? 0}
                        onChange={(event) => updateReceivingLine(line.returnLineKey, 'pendingClientQty', event.target.value)}
                        style={values.pendingClientQty > 0 ? { borderColor: '#f59e0b', background: '#fff7ed', color: '#92400e', fontWeight: 800 } : undefined}
                        title="Unidades que siguen físicamente con el cliente y todavía no se consideran faltantes"
                      />
                      ...
                      <span className="inventory-receiving-total">
                        ...
                        {values.pendingClientQty > 0 ? <small style={{ color: '#b45309' }}>Sin cargo por pendiente</small> : null}
                      ...
                      <input
                        ...
                        placeholder={values.pendingClientQty > 0 ? 'Qué quedó y cuándo se recogerá' : values.damagedQty > 0 || values.missingQty > 0 ? 'Detalle del daño/faltante' : 'Sin novedad'}
                      />
                    </div>
                  );
                })}
            ...
            <div className="inventory-legacy-modal-body">
              ...
              <section className="inventory-legacy-form-section inventory-legacy-economic-section">
                ...
                <div className="inventory-legacy-money-grid">
                  ...
                  <label><span>Garantía por cobrar</span><div className="inventory-money-input"><b>Bs</b><input type="number" min="0" step="0.01" placeholder="0,00" value={legacyForm.guaranteePendingBs} onChange={(e)=>setLegacyForm({...legacyForm,guaranteePendingBs:e.target.value})}/></div><small>Garantía que el cliente todavía debe pagar.</small></label>
      ...
      {legacyDetail ? (
        <div className="reset-modal-backdrop" onClick={()=>setLegacyDetail(null)}><section className="reset-modal inventory-legacy-detail-modal" onClick={(event)=>event.stopPropagation()}><header className="inventory-ops-all-head"><div><small>DOCUMENTO INDIVIDUAL · CONTRATO REZAGADO</small><h3>Contrato {legacyDetail.contractCode}</h3><p>{legacyDetail.customerName} · {legacyDetail.contractDate}</p></div><button type="button" className="orders-modal-close" onClick={()=>setLegacyDetail(null)}>x</button></header><div className="inventory-legacy-summary"><article><small>Saldo comercial</small><strong>{formatBs(legacyDetail.commercialDueBs||0)}</strong></article><article><small>Daños / pérdidas</small><strong>{formatBs(legacyDetail.damageDueBs||0)}</strong></article><article><small>Garantía por cobrar</small><strong>{formatBs(legacyDetail.guaranteePendingBs||0)}</strong></article><article><small>Garantía retenida</small><strong>{formatBs(legacyDetail.guaranteeHeldBs||0)}</strong></article><article><small>Por devolver</small><strong>{formatBs(legacyDetail.refundableGuaranteeBs||0)}</strong></article><article><small>Material pendiente</small><strong>{legacyDetail.pendingUnits||0} u.</strong></article></div><div className="inventory-legacy-table-wrap"><table className="inventory-legacy-table"><thead><tr><th>Ítem</th><th>Cantidad</th><th>Situación</th><th>Cargo</th><th>Acción</th></tr></thead><tbody>{(legacyDetail.items||[]).map(line=><tr key={line.id}><td>{line.itemName}</td><td>{line.quantity}</td><td>{line.status==='pending_return'?'Pendiente devolución':line.status==='missing'?'Faltante':line.status==='damaged'?'Dañado':line.status==='returned'?'Devuelto':'Resuelto'}</td><td>{formatBs(line.chargeBs||0)}</td><td>{line.status==='pending_return'&&line.quantity>0?<button type="button" className="primary-button" onClick={()=>receiveLegacyItem(legacyDetail,line)}>Recibir al stock</button>:['missing','damaged'].includes(line.status)?<button type="button" className="ghost-button" onClick={()=>resolveLegacyItem(legacyDetail,line)}>Marcar resuelto</button>:'-'}</td></tr>)}</tbody></table></div>{legacyDetail.notes ? <p className="status">{legacyDetail.notes}</p> : null}<p className="status">Las operaciones económicas se finalizan en Caja Grande y generan su movimiento y recibo.</p><div className="reset-modal-actions"><button type="button" className="ghost-button" onClick={()=>openLegacyEdit(legacyDetail)}>Editar</button><button type="button" className="ghost-button" onClick={()=>printLegacyContractDocument(legacyDetail)}>Imprimir / PDF</button><button type="button" className="primary-button" onClick={()=>setLegacyDetail(null)}>Cerrar</button></div></section></div>
      ) : null}
