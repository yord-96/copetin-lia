...
export const buildLincolnContractDocumentHtml = ({ event }) => {
  ...
  const extraRows = doc.extras.map((line) => {
    ...
    const scope = applicableGroups.length === groups.length ? 'TODO EL EVENTO' : applicableGroups.map((group) => group.name).join(' + ');
